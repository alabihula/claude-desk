...
describe('ChatComposer attachments', () => {
  ...
  it('opens the local MCP server panel from the slash menu without sending a message', async () => {
    ...
    expect(root.querySelector('.mcp-server-panel')?.textContent).toContain('Previous check: 2 tools (historical record)')
    expect(root.querySelector('.mcp-server-panel')?.textContent).toContain('figma-mcp-front')
    expect(root.querySelector('.mcp-server-panel')?.textContent).not.toContain('Unconfirmed')
  ...
  it('marks a missing MCP server as unknown after retry', async () => {
    ...
    expect(root.querySelector('.mcp-server-list')?.textContent).toContain('The check did not report a connection state')
